...
class service(object):
	...
	def __repr__(self):
		return "%s %s:%s" % (self.serviceName, str(self.serviceIP), self.servicePort)
...
class serviceFinder(object):
	...
	def __init__(self, ip, port):
		...
		self.sock.setsockopt(socket.IPPROTO_IP, socket.IP_MULTICAST_TTL, struct.pack('b', 33))
	...
	def search(self, serviceName):
		...
		while True:
			try:
				...
				if cmd == "service":
					servicesFound.append(service(serviceName, servicePort, serviceIP=server[0]))
